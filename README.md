...
The repository uses autocannon for deliberately guarded local performance checks. `npm run perf:seed` may replace only a local MongoDB database ending in `_perf` and requires `PERF_SEED_CONFIRM=synthetic`. `npm run perf:smoke`, `npm run perf:auth`, `npm run perf:employer`, and `npm run perf:write` never target a Production-like host by default and are intentionally excluded from normal CI. See [the reproducible local baseline](docs/performance/baseline.md) and [capacity guidance](docs/performance/capacity.md).
