...
## Applicant Application foundation
...
Applicant Application list/detail routes enforce the authenticated Applicant ID in their database filters and batch-load only safe Job/Company summary fields. Historical Applications remain readable when their Job later closes or archives; Job fields are deliberately live rather than fully snapshotted. Applicant snapshot download is deferred because current-resume access already covers the mutable resume and the Application snapshot is retained for future Employer-review evidence. Phase 8 must enforce Employer owns Company, Job belongs to that Company, and Application belongs to that Job before exposing any Application or snapshot.

## Deferred seams

S3/R2 resume storage alternatives, resume parsing/scanning, Employer Application review/status transitions/snapshot access, email verification/password reset delivery, OAuth, MFA, MongoDB Atlas/Search, Redis/BullMQ, caching, horizontal API replicas, CDN, and observability can be introduced behind future module boundaries. Application snapshot retention/account-erasure policy and production index rollout require explicit operational design. Full OpenAPI generation is deferred until business routes provide enough surface area. Persistent business schemas will require an explicit migration strategy. No distributed infrastructure is needed today.
