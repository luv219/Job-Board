...
export function createApplicationRouter(environment: Environment, storage: ResumeStorageProvider = createResumeStorageProvider(environment)): Router {
  const router = Router();
  ...
  router.post('/jobs/:jobId/applications', ...applicantOnly, validate('params', jobApplicationParamsSchema), validate('body', submitApplicationSchema), async (request, response, next) => {
    try { response.status(201).json({ application: await new ApplicationService(storage, request.log).submit(request.principal!.id, request.params.jobId as string, request.body as SubmitApplicationInput) }); }
  ...
  router.get('/applicant/applications', ...applicantOnly, validate('query', applicantApplicationListSchema), async (request, response, next) => {
    try { response.json(await new ApplicationService(storage, request.log).list(request.principal!.id, response.locals.validatedQuery as ApplicantApplicationListQuery)); }
    catch (error) { next(error); }
  });
  router.get('/applicant/applications/:applicationId', ...applicantOnly, validate('params', applicationIdParamsSchema), async (request, response, next) => {
    try { response.json({ application: await new ApplicationService(storage, request.log).get(request.principal!.id, request.params.applicationId as string) }); }
    catch (error) { next(error); }
  });
  router.post('/applicant/applications/:applicationId/withdraw', ...applicantOnly, validate('params', applicationIdParamsSchema), async (request, response, next) => {
    try { response.json({ application: await new ApplicationService(storage, request.log).withdraw(request.principal!.id, request.params.applicationId as string) }); }
    catch (error) { next(error); }
  });
  return router;
}
