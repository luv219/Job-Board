...
export function createEmployerApplicationRouter(environment: Environment, storage: ResumeStorageProvider = createResumeStorageProvider(environment)): Router {
  const router = Router();
  ...
  router.get('/employer/jobs/:jobId/applications', ...employerOnly, validate('params', jobApplicationParamsSchema), validate('query', employerApplicationListSchema), async (request, response, next) => {
    try { response.json(await new EmployerApplicationService(storage, request.log).listForJob(request.principal!.id, request.params.jobId as string, response.locals.validatedQuery as ApplicantApplicationListQuery)); }
  ...
  router.get('/employer/applications/:applicationId', ...employerOnly, validate('params', applicationIdParamsSchema), async (request, response, next) => {
    try { response.json({ application: await new EmployerApplicationService(storage, request.log).get(request.principal!.id, request.params.applicationId as string) }); }
    catch (error) { next(error); }
  });
  router.patch('/employer/applications/:applicationId/status', ...employerOnly, validate('params', applicationIdParamsSchema), validate('body', employerApplicationStatusSchema), async (request, response, next) => {
    try { response.json({ application: await new EmployerApplicationService(storage, request.log).transition(request.principal!.id, request.params.applicationId as string, (request.body as EmployerApplicationStatusInput).status) }); }
    catch (error) { next(error); }
  });
  router.post('/employer/applications/:applicationId/resume/access', ...employerOnly, validate('params', applicationIdParamsSchema), async (request, response, next) => {
    try {
      const access = await new EmployerApplicationService(storage, request.log).createSnapshotAccess(request.principal!.id, request.params.applicationId as string);
